...
flag = 0;
for ( i=laser_scan_msgs->ranges.size()*0.44 ; i<(laser_scan_msgs->ranges.size()*0.50); i++) {

if (laser_scan_msgs->ranges[i]<1) {
       flag = 1;
}
}
for ( i=laser_scan_msgs->ranges.size()*0.50 ; i<(laser_scan_msgs->ranges.size()*0.56); i++) {

if ((laser_scan_msgs->ranges[i]<1)&&(flag!=1)) {
       flag = 2;
}
}
